...
test('no comparison tasks defined', (t) => {
  t.test('default (record import) export: empty task list → merge with "No rules defined"', () => {
    const result = validateRecordMatch({record1Object: minimalRecord, record2Object: minimalRecord, comparisonTasks: []});
    assert.deepEqual(result, {action: 'merge', preference: {name: 'No rules defined', value: true}});
  });

  t.test('checkPreference: false with all tests passing → "all tests passed"', () => {
    const result = validateRecordMatch({
      record1Object: minimalRecord,
      record2Object: minimalRecord,
      checkPreference: false
    });
    assert.deepEqual(result, {action: 'merge', preference: {name: 'all tests passed', value: true}});
  });
});
